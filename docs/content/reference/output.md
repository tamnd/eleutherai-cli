...
describes what users get. Pick a format with `-o`, or let eai choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
eai <command> -o table   # aligned columns for reading
eai <command> -o jsonl   # one JSON object per line, for piping
eai <command> -o json    # a single JSON array
eai <command> -o csv     # spreadsheet friendly
eai <command> -o tsv     # tab-separated
eai <command> -o url     # just the URL column
eai <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
eai <command> --fields id,title,url
```
...
```bash
eai <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
eai <command>            # a table, because this is a terminal
eai <command> | wc -l    # JSONL, because this is a pipe
```
